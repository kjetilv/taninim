...
import mediaserver.taninim.music.Archives.ArchivedRecord;
import mediaserver.taninim.music.Leases;
import mediaserver.taninim.music.LeasesPath;
import mediaserver.taninim.music.LeasesRegistry;
import mediaserver.taninim.music.Period;
...
public final class ArchivedLeasesRegistry implements LeasesRegistry {
    ...
    private static List<Leases.Lease> leases(ArchivedRecord archivedRecord) {
        return archivedRecord.contents().stream().map(line -> line.split(" ", 2)).map(tokenLease -> new Leases.Lease(
